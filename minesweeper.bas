 ...
 760 LET %d = %c & INT{gridFlagsMask} >> 4
 770 ; IF v = 0 THEN LET c$=" ": PAPER 4:; green
 780 ; IF %c & INT {mineFlag} THEN LET c$="X": PAPER 2:; red
 790 ; IF %c & INT {uncoveredFlag} = 0 THEN LET c$  = " ": PAPER 7:; white
 800 ; IF %c & INT {flaggedFlag}>0 THEN IF %c & INT {uncoveredFlag}=0 THEN LET c$=" ": PAPER 3:; magenta
